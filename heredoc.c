...
static int	heredoc_read_loop(int *pipefd, char *delim, t_env *env)
{
	...
	while (!g_signal)
	{
		...
		if (ret == EOF_HEREDOC | ret == ERROR_HEREDOC)
			return (ret);
	}
...
int	handle_heredoc(t_cmd **commands, char **array, t_env *env)
{
	...
	if (pipe(pipefd) == -1)
		return (perror("pipe"), free_and_null(&delimiter), init_signals(),
			ERROR_HEREDOC);
	...
	init_signals();
	if (heredoc_output(commands, pipefd[0]) == ERROR_FILE)
		return (close(pipefd[0]), ERROR_FILE);
